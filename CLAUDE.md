...
### "The update didn't take" — now answerable (2026-08-22)

`_raw_refresh()` used to swallow every failure in a bare `except: continue`, then print a
success line driven by `got_main` — so a 404, an expired cert or a locked file produced no
message at all, and it announced "refreshed core files" even when eight of nine had
failed. Nothing validated a download either, so a captive-portal or proxy login page
returning HTTP 200 was written straight over a working `.py`.

Now every download is parsed before it lands (`compile()` for `.py`, `json.loads()` for
`.json`) and anything that does not parse is **refused, with the working copy kept**.
Writes go to a temp file and swap in via `os.replace`, so a crash mid-write cannot leave a
half-written core file. Each file reports **updated / already current / FAILED with the
reason**, and any failure raises the same loud banner the git path uses.

`_deploy_manifest()` then prints, right after the update, the mtime and byte fingerprint
of every core file actually on disk — plus `MISSING` for any core file absent from that
PC. Nothing in it is hand-maintained, so it cannot go stale the way `BUILD_DATE` did.
**That block is the answer when somebody says the update didn't take.**
