...
echo [6/7] Google creds (download + verify the real key)...
python -c "import os,urllib.request,json; p=os.path.join(os.path.expanduser('~'),'optimus','google_creds.json'); urllib.request.urlretrieve('https://drive.google.com/uc?export=download&id=%CREDS_ID%', p); d=json.load(open(p)); assert d.get('project_id')=='fiberscanner-493900','bad'; print('  creds OK ->', p)" || (echo   Auto creds download did not return the real key - the hunter will use any good copy already on this PC, else sheet-writes stay off. & del "%HOME%\google_creds.json" 2>nul)
